...
namespace Skeleton\BasketTeam\Infrastructure\BasketTeam\UI\Messaging\Consumers;

use OldSound\RabbitMqBundle\RabbitMq\ConsumerInterface;
use PhpAmqpLib\Message\AMQPMessage;
use Skeleton\BasketTeam\Infrastructure\BasketTeam\UI\Messaging\Subscribers\BasketTeamPlayerWasAddedSubscriber;
use Skeleton\BasketTeam\Infrastructure\BasketTeam\UI\Messaging\Subscribers\BasketTeamPlayerWasRemovedSubscriber;
use Symfony\Component\DependencyInjection\ContainerInterface;

final class BasketTeamConsumer implements ConsumerInterface
{
    private const MAX_RETRIES = 5;
    private const TIME_TO_RETRY = 10000;

    const EVENT_SUBSCRIBERS = [
        'skeleton.basketTeam.event.1.player.added' => [BasketTeamPlayerWasAddedSubscriber::class],
        'skeleton.basketTeam.event.1.player.removed' => [BasketTeamPlayerWasRemovedSubscriber::class],
    ];
    ...
    private function retry(AMQPMessage $msg, $data): bool
    {
        ...
        if ($data && self::MAX_RETRIES >= $retries && isset($data['headers']['name'])) {
            $this->container->get('old_sound_rabbit_mq.basketTeamDelay_producer')->publish(
                json_encode($data),
                $data['headers']['name'],
